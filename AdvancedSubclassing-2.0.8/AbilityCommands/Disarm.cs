		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			...
			if (!subClass.BoolOptions.ContainsKey("DisarmSphere") || !subClass.BoolOptions["DisarmSphere"])
			{
					...
					(subClass.FloatOptions.ContainsKey("DisarmRange") ? subClass.FloatOptions["DisarmRange"] : 1.3f)))
				{
					...
					if (target == null || target.Id == player.Id || player.Role.Side == target.Role.Side || (player.Role.Team == Team.SCP && target.Role.Team == Team.TUT)) return true;
			...
			else
			{
				...
				if (players.Count() > 0)
				{
					...
					{
						if (target == null || target.Id == player.Id || player.Role.Side == target.Role.Side || (player.Role.Team == Team.SCP && target.Role.Team == Team.TUT)) continue;
						if (target.CurrentItem != null) target.DropItem(target.CurrentItem);
					}
				}
			}
