...
namespace Subclass
{
	public static class TrackingAndMethods
	{
		...
		public static void AddClass(Player player, SubClass subClass, bool is035 = false, bool lite = false, bool escaped = false, bool disguised = false)
		{
			...
			spawnLocations.AddRange(Room.List.Where(r => subClass.SpawnLocations.Contains(r.Role.Type.ToString())).Select(r => r.Transform.position));
			...
			while (!(subClass.SpawnLocations[spawnIndex] == "Unknown" || subClass.SpawnLocations[spawnIndex] == "Lcz173Armory" || subClass.SpawnLocations[spawnIndex] == "Lcz173"
				...
				&& !Room.List.Any(r => r.Effect.Type.ToString() == subClass.SpawnLocations[spawnIndex]))
			{
		...
		public static bool AllowedToDamage(Player t, Player a)
		{
			...
			if (attackerClass != null && (attackerClass.CantDamageRoles.Contains(t.Role) || attackerClass.CantDamageTeams.Contains(t.Role.Team == Team.TUT ? Team.SCP : t.Role.Team))) return false;
			if (targetClass != null && (targetClass.RolesThatCantDamage.Contains(a.Role) || targetClass.TeamsThatCantDamage.Contains(a.Role.Team == Team.TUT ? Team.SCP : a.Role.Team))) return false;
		...
		public static IEnumerator<float> CheckRoundEnd()
		{
			...
			if (uniqueTeamsAlive.Count == 2 && uniqueTeamsAlive.Contains("SCP"))
			{
				...
				if (Player.List.Where(p => p.Role.Team == Team.SCP).All(p => zombies.Contains(p)))
				{
			...
			if (PlayersWithSubclasses != null && PlayersWithSubclasses.Count(s => s.Value.EndsRoundWith != "RIP") > 0)
			{
				foreach (Player player in PlayersWithSubclasses.Keys)
				{
					if ((PlayersWithSubclasses[player].BoolOptions.ContainsKey("ActAsSpy") && PlayersWithSubclasses[player].BoolOptions["ActAsSpy"]) &&
						...
						PlayersWithSubclasses[player].EndsRoundWith != player.Role.Team.ToString() &&
		...
		{
			List<string> teamsAlive = Player.List.Select(p1 => p1.Role.Team.ToString()).ToList();
			teamsAlive.RemoveAll(t => t == "RIP");
			foreach (var item in PlayersWithSubclasses.Where(s => s.Value.EndsRoundWith != "RIP"))
			{
				teamsAlive.Remove(item.Key.Role.Team.ToString());
